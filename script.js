...
// --- VISUAL AESTHETICS (Laser Look) ---
const VISUAL_COLOR = '#38bdf8'; // Cyberpunk Cyan
const VISUAL_GLOW_COLOR = 'rgba(56, 189, 248, 0.6)';
const VISUAL_GLOW_SIZE = 20;
const VISUAL_STROKE_WIDTH = 16;
...
async function initApp() {
    ...
    try {
        ...
        console.error(e);
        updateStatus("MODEL_OFFLINE", "error");
        alert("Error: Run python -m http.server");
    }
}
...
function initCanvas() {
    ...
    // 1. Visual Context Style (Glowing Cyan)
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
...
function move(e) {
    ...
    // 1. Draw Visual (Gap-free Line)
    ctx.beginPath();
    ctx.moveTo(lastX, lastY);
    ctx.lineTo(x, y);
    ctx.stroke();

    // 2. Draw Hidden (Gap-free Line)
    hiddenCtx.beginPath();
...
function getBoundingBox(data, w, h) {
    ...
    for(let y=0; y<h; y++) {
        for(let x=0; x<w; x++) {
            // Threshold 50 to detect white ink
            if(data[(y*w + x)*4] > 50) {
                if(x < minX) minX=x;
                if(x > maxX) maxX=x;
                if(y < minY) minY=y;
...
function extractInputTensor(shiftX = 0, shiftY = 0) {
    // READ FROM HIDDEN (White on Black)
    ...
    for(let i=0; i<finalData.length; i+=4) {
        ...
        val = Math.min(1.0, val * 1.3); // Signal boost
        input.push(val);
    }
...
predictBtn.addEventListener('click', async () => {
    ...
    if(max > 0.9) confidenceBar.style.background = 'var(--success)';
    else if(max > 0.7) confidenceBar.style.background = 'var(--accent)';
    else confidenceBar.style.background = '#fbbf24';
    ...
    if(contendersList) {
        ...
        sorted.forEach(item => {
            ...
            row.style.cssText = "display:flex; align-items:center; gap:10px; margin-bottom:6px; font-family:var(--font-mono); font-size:0.8rem;";

            row.innerHTML = `
                <span style="width:15px; color:var(--text-dim);">${item.i}</span>
                ...
                <span style="width:45px; text-align:right; color:var(--accent);">${pct}%</span>
            `;
